...
Memories are scoped to the signed-in user. API and MCP access use the authenticated account, and cross-user object access is designed to fail without exposing another user's data.
...
An AI tool connected to Memside can only work with the permissions granted through the connection method. If you disconnect a tool or revoke an API key, that client is no longer authorized to call Memside with that credential.
